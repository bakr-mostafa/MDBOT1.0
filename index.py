...
app.config["UPLOAD_FOLDER"] = os.getcwd() + "/static/user_images"
...
@app.route('/dent-test',  methods=['POST','GET'])
def dent_test():        
    if request.method == 'POST': 
        ...
        if file and allowed_file(file.filename):
            ...
            if filename in os.listdir(app.config['UPLOAD_FOLDER']):
                ...
                return render_template('dent-test.html', prediction = pred)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename)) 
            pred = teeth_predict.predict(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            return render_template('dent-test.html', prediction = pred)
    
    return render_template('dent-test.html')
